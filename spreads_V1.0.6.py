...
# chave planilha teste111 credentials = ServiceAccountCredentials.from_json_keyfile_name('chave.json', scope)  # validacao da chave
...
# Abre a planilha
# planilha teste111 wks = gc.open_by_key('1SXvafCDkFjrDAp5fZSRmRYlJZ9LeUMS3r8dF7KEmAPQ')  # endereço da planilha
...
def hilabs_encontrados():
    ...
    while n != 1:
        ...
        if not (nome in nomes):
            ...
            pessoas.append(worksheet.cell(n, 6).value) # talvez um delay aqui!!
...
def funcao_pausar_atividade_pre(indice):
    ...
    if quem_pausou[indice] == 0:
        ...
        worksheet.update_acell(
            ...
            'M' + str(linha.row), tempo_pausado_anterior)

        motivo = input("Digite o motivo da parada: ")
        ...
        button1 = tk.Button(window, width=10, text="Enviar", command=getvalue).grid(row=5, column=10)
...
if processo == "Fabricação":
    ...
    abertos()
    hilabs_encontrados()
    nomes_atualizados = [tira_ponto(hilab) for hilab in nomes]
    while contador_hilabs <= demanda:
        cara = "HILAB-" + str(contador_hilabs)
        if cara in nomes_atualizados:
            atividade_agora.append(atividades[nomes_atualizados.index(cara)])  #escreve em atividade_agora a atividade q ele esta
            ...
            contador_hilabs += 1
            if cara in aberto:
                T_inicial = tempo_inicial[aberto.index(cara)]
                horario_do_inicio_de_atividade.append(T_inicial)
        else:
            listadehilabs.append(cara)
            contador_hilabs += 1
            quem_pausou.append(0)
            tempo_pausado.append(0)
    ...
    lista_de_fixacoes = ["Elevador", "Base", "Tampa"]
    ...
    while indice < len(lista_de_pre_fab):
        ...
        opcoesfabricacao = tk.OptionMenu(window, optionsfabricacao, "OPF0043/2021 (capsense)",
                                         "OPF0044/2021 (eletroima)",
                                         "OPF0045/2021 (barra direita)",
                                         "OPF0046/2021 (barra esquerda)",
                                         "OPF0047/2021 (elevador)",
                                         command=salvarfabricacao)  # config da combobox de opf
