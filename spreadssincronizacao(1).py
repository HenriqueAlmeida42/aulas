...
def iniciar_atividade_pre(indice):  # botao de iniciar atividade que ainda estamos construindo (ele so vai salvar o horario atual quando clica
    ...
    if finalizar[indice] == 0:
        ...
        linha = worksheet.find(lista_de_pre_fab[indice])
...
def funcao_pausar_atividade_pre(indice):
    ...
    if quem_pausou[indice] == 0:
        ...
        bname.configure(text='Pausar ' + atividade_agora[indice])
        linha = worksheet.find(lista_de_pre_fab[indice])
        worksheet.update_acell('K' + str(linha.row), str(tempo_pausado[indice]).split(".")[0])
...
if processo == "Fabricação":
    ...
    while indice < len(lista_de_pre_fab):
        ...
        abas_pre_fabricacao_no_dashboard = tk.Label(window, text=fabricacao,
                                  width=30)  # puxa o status pela key do dict de hilabs
