...
while 1==1:
    ...
    ordemdefabricacao = ["Iniciar", "Montagem do termo bloco", "Montagem Chassi", "Montagem Base", "Montagem Final",
                         "Registro do equipamneto", "Ajuste de Temperatura"]  # Ordem dos procesos que utilizaremos como guia para os botoes de atividades da fabricação
    listadepolvos = [] #listadehilabs
    ordem_pre_fabricacao = ["Iniciar", "Fabricação"]
    atividade_agora = []
    nome_dos_polvos_da_planilha = []  # nome_dos_hilabs_da_planilha
    atividades_dos_nomes_dos_polvos_da_planilha = []  #atividades_dos_nomes_dos_hilabs_da_planilha
    quem_pausou = []
    ...
    versoes_e_lotes_titulo = ["Etiqueta POLVO", "Lote da Placa N0:", "Lote da Placa N1:", "Lote da Placa N2:",
                              "Lote da Placa N3:","Lote da Placa N4:",
                              "Lote da Tampa Inferior:", "Lote da Tampa Superior:","Lote NFC", "Lote Termobloco"]
    versoes_e_lotes = [0, 0, 0, 0, 0, 0, 0, 0, 0,
    ...
    def hilabs_encontrados():
        ...
        indice_de_polvo = tamanho - 1  # indice_de_hilab
        print("coluna", coluna_de_materiais)
        while indice_de_polvo >= 0:  # indice_de_hilab
            if "POLVO-" in coluna_de_materiais[indice_de_polvo]:  #indice_de_hilab
                print(coluna_de_materiais[indice_de_polvo], indice_de_polvo)
                if coluna_de_materiais[indice_de_polvo].replace(".", "") in nome_dos_polvos_da_planilha:  #nome_dos_hilabs_da_planilha
                    indice_de_polvo -= 1
                    continue

                nome_dos_polvos_da_planilha.append(coluna_de_materiais[indice_de_polvo].replace(".", ""))  # nome_dos_hilabs_da_planilha
                if "." in coluna_de_materiais[indice_de_polvo]:
                    finalizar.append(0)

                    proxima_atividade = ordemdefabricacao.index(coluna_de_atividades[indice_de_polvo])
                    atividades_dos_nomes_dos_polvos_da_planilha.append(ordemdefabricacao[proxima_atividade + 1])  #atividades_dos_nomes_dos_hilabs_da_planilha
                    quem_pausou.append(0)
                    tempo_pausado.append(0)
                    horario_do_inicio_de_atividade.append(0)
                    horario_do_final_de_atividade.append(0)
                    opf_de_cada_polvo.append(coluna_de_opfs[indice_de_polvo])
                else:
                    atividades_dos_nomes_dos_polvos_da_planilha.append(coluna_de_atividades[indice_de_polvo])  #atividades_dos_nomes_dos_hilabs_da_planilha
                    finalizar.append(1)
                    horario_do_inicio_de_atividade.append(coluna_horario_inicial[indice_de_polvo])
                    horario_do_final_de_atividade.append(0)
                    quem_pausou.append(0)
                    tempo_pausado.append(0)
                    opf_de_cada_polvo.append(coluna_de_opfs[indice_de_polvo])
            ...
            indice_de_polvo -= 1
        print(nome_dos_polvos_da_planilha, atividades_dos_nomes_dos_polvos_da_planilha)
    ...
    def iniciar_atividade_e_atualizar_status_do_hilab( #iniciar_atividade_e_atualizar_status_do_hilab
        ...
        if finalizar[indice] == 0:
            ...
            worksheet.update_acell('A' + linha, listadepolvos[indice]) #listadehilabs[indice
            ...
            print(tempo)
            linha = worksheet.find(listadepolvos[indice])  # (listadehilabs[indice])
            worksheet.update_acell('D' + str(linha.row), tempo)
            worksheet.update_acell('A' + str(linha.row), listadepolvos[indice] + ".")  # listadehilabs[indice] + ".")

            if (atividade_agora[indice] == "Registro do equipamneto"):  # check-list

                if (versoes_e_lotes[8] == 0):
                    contador_de_lotes = 0
                    print(f"esse é o tamanho de versoes e lotes {len(versoes_e_lotes)}")
                ...
                elif (atividade_agora[indice] == "Ajuste de Temperatura"):  # Ajuste de Temperatura
                    def getvalue():
                        global versoes_e_lotes
                ...
                worksheet3.update_acell("B" + linha3, opf_de_cada_polvo[indice])
                worksheet3.update_acell("C" + linha3, versoes_e_lotes[0])
                worksheet3.update_acell("D" + linha3, "Ajuste de Temperatura")  # check-list
                ...
                worksheet3.update_acell("N" + linha3, versoes_e_lotes[9])
                ...
                while contador_troca < len(vamos_trocar_todos_os_hilabs_temporarios):
                    if listadepolvos[indice] == vamos_trocar_todos_os_hilabs_temporarios[contador_troca].replace(".",""):
    ...
    def funcao_pausar_atividade(indice):
        ...
        print(bname, indice, listadepolvos[indice])
        ...
        if quem_pausou[indice] == 0:
            ...
            linha = worksheet.find(listadepolvos[indice])
    ...
    def funcao_refresh_hilab():
        ...
        tamanho = len(coluna_de_materiais)
        indice_de_polvo = tamanho - 1  # indice_de_hilab
        while indice_de_polvo>= 0:
            if "POLVO-" in coluna_de_materiais[indice_de_polvo]:
                print(coluna_de_materiais[indice_de_polvo], indice_de_polvo)
                if coluna_de_materiais[indice_de_polvo].replace(".", "") in nome_dos_polvos_da_planilha:  #nome_dos_hilabs_da_planilha
                    indice_de_polvo -= 1
                    continue
        ...
        finalizar = []
        atividade_agora = []
        global listadepolvos
        while contador_ordenar_hilabs_da_planilha < len(nome_dos_polvos_da_planilha):  # nome_dos_hilabs_da_planilha
            if listadepolvos[contador_ordenar_hilabs_da_planilha] in nome_dos_polvos_da_planilha:  #nome_dos_hilabs_da_planilha
                atividade_agora.append(atividades_dos_nomes_dos_polvos_da_planilha[nome_dos_polvos_da_planilha.index(  #atividades_dos_nomes_dos_hilabs_da_planilha; nome_dos_hilabs_da_planilha
                    listadepolvos[contador_ordenar_hilabs_da_planilha])])
                finalizar.append(finalizar_refresh[
                                     nome_dos_polvos_da_planilha.index(listadepolvos[contador_ordenar_hilabs_da_planilha])])# nome_dos_hilabs_da_planilha
            contador_ordenar_hilabs_da_planilha += 1

        print(finalizar)
        print(listadepolvos, atividade_agora)
        while indice_refresh < len(finalizar):
            if finalizar[indice_refresh] == 1:  # se tiver pra finalziar, configura o botao pra finalizar
                ...
                print("finalizar = 1", listadepolvos[indice_refresh])
    ...
    if processo == "Fabricação":
        ...
        if iniciarouresume == "Iniciar":
            ...
            while contador_hilabs <= demanda:
                listadepolvos.append("POLVO-" + str(contador_hilabs))
                contador_hilabs += 1
                quem_pausou.append(0)
                tempo_pausado.append(0)
                horario_do_inicio_de_atividade.append(0)
            ...
            while indice < len(listadepolvos):
                abasfabricacao = tk.Label(second_frame, text=listadepolvos[indice],
            ...
            hilabs_encontrados()
            listadepolvos = nome_dos_polvos_da_planilha  # nome_dos_hilabs_da_planilha
            atividade_agora = atividades_dos_nomes_dos_polvos_da_planilha  # atividades_dos_nomes_dos_hilabs_da_planilha

            indice = 0

            while indice < len(nome_dos_polvos_da_planilha):  # nome_dos_hilabs_da_planilha
                abasfabricacao = tk.Label(second_frame, text=listadepolvos[indice],
                                          width=10)  # nome do hilab do indice pela lista convertida
                abasfabricacao.grid(row=(indice + 1), column=1)
                abasfabricacao = tk.Label(second_frame, text=fabricacao,
    ...
    # Polvo mudança no nome das atividades pré
        opcoes_pre_fabricacao = tk.OptionMenu(second_frame, options_pre_fabricacao, "Solda", "Coolerfan",
                                              "Termobloco", "Cartucho de Calibração", command=salvar_pre_fabricacoes)
        ...
        while indice_opfs < len(lista_de_opfs):
            ...
            indice_opfs += 1
        print(opfsdodia)
    #Polvo mudança nas atividades das prés
        lista_de_soldas = ["NTC", "Inspeçao NTC", "NFC", "Inspeção NFC e Ligação 12C",
                           "Barra de Led","Inspeção Barra de Led",
                           "Cabo do Aquecedor", "Inspeçao Cabo do Aquecedor",
                           "Contagem"]
        ...
        lista_de_cartucho = ["Contagem", "Montagem do Cartucho", "Inspeção e Gravação Tag"]
        ...
        elif qual_pre_fabricacao == "Cartucho de Calibração":
            lista_de_pre_fab = lista_de_cartucho
        contador_pre = 1
        ...
        if linha_retrabalho > 2:
            while linha_retrabalho > (linha_maxima - 2):
