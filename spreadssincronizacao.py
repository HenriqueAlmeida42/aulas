...
def iniciar_atividade_pre(indice):  # botao de iniciar atividade que ainda estamos construindo (ele so vai salvar o horario atual quando clica
    ...
    if finalizar[indice] == 0:
        ...
        if (indicedeatividade+1) < len(ordem_pre_fabricacao):
            ...
            escrever_quantidade_final = tk.Entry(window)
            escrever_quantidade_final.grid(row=(indice + 1), column=3)
            print(escrever_quantidade_final.get())
            quantidade_finalizada = escrever_quantidade_final.get()
            continue_button = tk.Button(window, text='Enviar', command=continuarcomretorno(quantidade_finalizada))
            continue_button.config(width=10)

            # set the coordinates as you want. here 2,6 for the example
            continue_button.grid(row=12, column=7)
            window.mainloop()
