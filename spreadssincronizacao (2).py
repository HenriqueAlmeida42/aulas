...
def funcao_pausar_atividade_pre(indice):
    ...
    if quem_pausou[indice] == 0:
        ...
        worksheet.update_acell('K' + str(linha.row), str(tempo_pausado[indice]).split(".")[
            0])  # inclui o valor da parada na linha de tempo parado
        motivo = input("Digite o motivo da parada: ")
        worksheet.update_acell('L' + str(linha.row), motivo)
...
if processo == "Fabricação":
    ...
    while indice < len(lista_de_pre_fab):
        ...
        abas_pre_fabricacao_no_dashboard = tk.Label(window, text=fabricacao,
                                                    width=20)  # puxa o status pela key do dict de hilabs
        abas_pre_fabricacao_no_dashboard.grid(row=(indice + 1), column=2)
