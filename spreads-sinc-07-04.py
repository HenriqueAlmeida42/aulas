...
def iniciar_atividade_pre(
    ...
    bname = (identidade_do_botao_de_iniciar_atividade[indice])
    print(bname, indice)
    demanda = worksheetopf.acell("F" + str((worksheetopf.find(fabricacao[3:7])).row)).value

    todas_as_opfs_iguais = worksheet.findall(fabricacao)
...
botao_refresh.grid(row=indice + 1, column=3)
window.mainloop()
